...
class InternationalPublicationController extends Controller
{
    public function index()
    {
        
        $categories = Category::select('id', 'name')->orderby('id', 'DESC')->get();
        $data = Publication::select('id', 'name', 'description', 'category_id', 'is_featured', 'is_recent', 'is_popular', 'is_trending', 'feature_image', 'slug',  'status')->orderby('id','DESC')->get();

        return view('admin.publication.index', compact('data','categories'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $chkSlug = Publication::where('slug', $pslug)->exists();
        if ($chkSlug) {
            $pslug = $pslug . '-' . mt_rand(10000000, 99999999);
        }

        $product = new Publication;
    ...
    {
        $info = Publication::where('id', $id)->first();
    ...
    {
       $validator = Validator::make($request->all(), [
            'name' => 'required|string|max:255|unique:publications,name,' . $request->codeid,
            'short_description' => 'nullable|string',
            'description' => 'required|string',
            'is_featured' => 'nullable',
            'is_recent' => 'nullable',
        ...
        $product = Publication::find($request->codeid);
    ...
    public function toggleStatus(Request $request)
    {
        ...
        $product = Publication::find($request->id);
