...
class EssayController extends Controller
{
    public function getEssay()
    {
        ...
        $data = Essay::select('id', 'name', 'description', 'category_id', 'is_featured', 'is_recent', 'is_popular', 'is_trending', 'feature_image', 'slug',  'status')->orderby('id','DESC')->get();

        return view('admin.essay.index', compact('data','categories'));
    }
    ...
    public function essayStore(Request $request)
    {
        ...
        $chkSlug = Essay::where('slug', $pslug)->exists();
        if ($chkSlug) {
    ...
    public function essayEdit($id)
    {
        $info = Essay::where('id', $id)->first();
    ...
    public function essayUpdate(Request $request)
    {
       $validator = Validator::make($request->all(), [
        ...
        $product = Essay::find($request->codeid);
    ...
    public function essayDelete($id)
    {
        $product = Essay::find($id);
