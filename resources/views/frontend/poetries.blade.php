...
$data = \App\Models\Poetry::select('id','slug', 'description', 'name','feature_image','short_description')->orderby('id', 'DESC')->get();
@endphp
<!-- Content Section -->
<main class="content">
                    ...
                    @if ($loop->iteration % 2 == 0)
                        ...
                        <div class="col-md-6">
                            <h2 class="fw-bold title">
                                <a href="{{ route('poetries.show', $story->slug) }}">{{ $story->name }}</a>
                            </h2>
                            <p>
                                {!! Str::limit(strip_tags($story->description), 200) !!}
                                <a href="{{ route('poetries.show', $story->slug) }}" class="btn btn-link">See More</a>
                            </p>
                        </div>
                    @else
                        <!-- Text Left -->
